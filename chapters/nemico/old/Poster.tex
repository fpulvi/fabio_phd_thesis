...
Traffic analyses are commonly performed on: (i) the packet payloads, (ii) the traffic metrics, or (iii) some statistical features computed on traffic flows. 
...
the discovery of significant correlations among data~\cite{NostroComNet}, the extraction of kowledge useful for prediction~\cite{KaragiannisPF05}, and 
...
More specifically, this paper presents \Nemico , a data mining system focused on efficiently discovering interesting knowledge from Big network datasets by means of distributed approaches.
\Nemico\ consists of a series of distributed MapReduce jobs running in the cloud. Each job performs a different step of the knowledge discovery process, ranging from the network data acquisition to knowledge exploitation. 
...
The system has been thought to support the integration a variety of data mining algorithms, including supervised approaches (e.g., classification and regression algorithms) and unsupervised ones (e.g., association rule mining, clustering).
...
Each job takes as input the result of one or more preceding jobs and it performs a specific step of network data mining process.
...
\comment{\textbf{Labelling}. Supervised data mining techniques (e.g., classification) require the labelling of one or more data attributes 
...
Since many data mining algorithms (e.g.,~\cite{ISPA14}) allow us to push taxonomy-based information into the extraction process, \Nemico\ supports both
the automatic taxonomy inference over a subset of specific network data attributes (e.g., for the considered attributes (e.g., port number, packet number) 
...
The current implementation of \Nemico\ includes a first implementation of all described activities as map jobs.
...
The proposed algorithms are designed to address the following open issues.
...
to select top-k interesting ones or cateogrized into semantically related groups (e.g., all patterns related to a specific class of service).
...
The current implementation of \Nemico\ was developed in Java using the new Hadoop Java APIs. It has been validated on real network traffic datasets with size 192.56 GB. The experiments were performed on a cluster of 5 nodes running Cloudera's
Distribution of Apache Hadoop (CDH4.5). More details of both cluster node and dataset can be found in \cite{ISPA14}. 
For both data mining algorithms in \Nemico , we evaluated the speedup achieved increasing the number of Hadoop cluster nodes (i.e., 1, 3 , and 5). The achieved results show that our approaches scale roughly
linearly with the number of nodes and the speedup approximately
