...
%Here we focus on itemset mining algorithms because they
...
%frequent itemset mining problem, introducing the main algorithmic approaches of the domain.  Section~\ref{survey_parallelization} introduces the strategies adopted to partition and parallelize the problem of frequent itemset mining. Section~\ref{survey_algorithms} discusses
%the selected algorithms, while in
%Section~\ref{survey_experimental} we benchmark the algorithms with a large set of
%experiments on both real and synthetic datasets.
%Section~\ref{survey_openissues} discusses the open questions and issues raised by the experimental validation of the theoretical analysis, highlighting some possible research directions to
%be addressed to support a more effective and efficient data mining process on
%big data collections
%Section~\ref{survey_lesson} summarizes the more concrete lessons learned from our evaluation
...
%evaluation criteria considered in this study. Section~\ref{survey_algorithms} discussed
%the selected algorithms, while in
%Section~\ref{survey_experimental} we benchmark the algorithms with a large set of
%experiments on both real and synthetic datasets.
%Section~\ref{survey_lesson} summarizes the lessons learned from our evaluation
%analysis, while Section~\ref{survey_openissues} discusses some research directions to
%be addressed to support a more effective and efficient data mining process on
...
FP-growth suits well dense datasets, because they can be effectively and compactly represented by means of the FP-tree data structure. Differently, with sparse datasets, the compressions benefits of the FP-tree are reduced because there will be a higher number of branches \cite{Han_2007SurveyFIM} (i.e., a large number of subproblems to generate and results to merge).
...
%
%\hline {\bf Criterion} & {\bf Approach~1 (Figure~\ref{survey_approach1noniterativo})} & {\bf Approach~1 - Iterative version (Figure~\ref{approach1})} & {\bf Approach~2 (Figure~\ref{approach2})} \\
%\hline
...
%In fact, the parallel Apriori-based algorithm used by BigFIM, based on the first parallelization strategy introduced in Section~\ref{survey_parallelization}, 
...
%(iii)~communication costs (Section~\ref{survey_communication_costs}), MLlib PFP results as the more balanced algorithm while Mahout PFP the best in terms of communication costs. \\
...
% datasets in Table~\ref{survey_datasets_transactions} were used to study the performance
...
% Table~\ref{survey_datasets_real} are real dataset used to simulate real life use cases.
...
% (~\ref{survey_datasets_transactions}).
...
%%\begin{table*}[h!]
%%\begin{center}
%%\caption{Experimental result summary for $minsup$ impact, Section~\ref{survey_minsup_exp}}
%%\label{minsup_resume}
%%\begin{tabular}{|c|c|c|c|c|}
%%\hline
...
%\begin{table*}[h!]
%\begin{center}
%\caption{Experimental result summary for transaction length impact, Section~\ref{survey_attributes_exp}}
...
% \ref{survey_minsup_exp}, we have performed a set of experiments dealing with datasets
...
% \ref{survey_minsup_exp},  it is unable to complete the mining task when
...
%This experiment, because of the size of the involved input dataset, should be very related to the behavior in terms of communication costs of the algorithms. As we will detail in Subsection~\ref{survey_communication_costs}, Mahout PFP implemention is, by far, the algorithm assuming less data across the network. However, in this experiment, the performance of Mahout PFP are not the best-in-class. For the same reason, even the gap with the performances of BigFIM is not that large (since the communication costs balance differs, instead, by a factor larger than four).
%\begin{table*}[h!]
%\begin{center}
%\caption{Experimental result summary for number of transactions impact, Section~\ref{survey_transaction_exp}}
...
Figure~\ref{survey_nr_machines_speedup} shows the speedup results. 
...
%In Figure~\ref{survey_nr_machines_speedup}, the execution time of the basic configuration are divided by the ones related to the higher degree of parallelization to obtain the speedup values.
 % (Figure~\ref{nr_machines_1}). 
%Since the problem was already too challenging for a single task, we have started from 2. In Figure~\ref{nr_machines_1} and Figure~\ref{survey_nr_machines_speedup} are shown the performance of the algorithms leveraging different parallelization degree values.
...
% ~\ref{survey_attributes_exp} with Mahout PFP: a single task, with an execution period
...
%
%In Section~\ref{survey_minsup_exp}, measuring the impact of \textbf{minsup threshold}, we have seen that, as expected, the first phase of BigFIM is fundamental for its scalability. 
%It represents the failure point when the number of candidate itemsets is too large to be stored in the main memory of the mappers. 
...
%evaluation discussed in Section~\ref{survey_experimental},
